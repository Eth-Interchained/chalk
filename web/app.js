...
async function drawShareCard(canvas) {
  ...
  // six ratings strip
  // Six tiles, always: width derived from the space, not a constant (v0.12.9 — 172 px tiles only fit four, and the
  // overflow guard silently dropped Explosiveness and Ball Security from Mark's first live card).
  const list = (h?.ratings || []).slice(0, 6); const gap = 8, x0 = 330, y0 = 470, avail = SHARE_W - 24 - x0;
  const tileW = Math.floor((avail - gap * (list.length - 1)) / Math.max(1, list.length));
  const labelFont = tileW < 150 ? 13 : 15, scoreFont = tileW < 150 ? 34 : 40, rankFont = tileW < 150 ? 12 : 14;
  list.forEach((r, i) => { const x = x0 + i * (tileW + gap); ctx.fillStyle = r.subject === sj ? accent + "26" : "rgba(255,255,255,.07)"; roundRect(ctx, x, y0, tileW, 92, 14); ctx.fill(); if (r.subject === sj) { ctx.strokeStyle = accent; ctx.lineWidth = 1.5; ctx.stroke(); } ctx.fillStyle = "rgba(255,255,255,.62)"; ctx.font = `700 ${labelFont}px ${D}`; let lbl = r.label.toUpperCase(); while (ctx.measureText(lbl).width > tileW - 20 && lbl.length > 4) lbl = lbl.slice(0, -2) + "…"; ctx.fillText(lbl, x + 10, y0 + 10); ctx.fillStyle = "#fff"; ctx.font = `900 ${scoreFont}px ${D}`; ctx.fillText(String(r.score ?? "–"), x + 10, y0 + 30); ctx.fillStyle = "rgba(255,255,255,.55)"; ctx.font = `500 ${rankFont}px ${M}`; ctx.fillText(`#${r.rank} of ${r.of}`, x + 10, y0 + 70); });
  // footer
