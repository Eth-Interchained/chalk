...
before(async () => {
  ...
  store = new ChalkStore({ url: `http://127.0.0.1:${port}`, db: "chalk_test" });
  const deadline = Date.now() + 15000;
  while (Date.now() < deadline) {
    if (await store.ping()) return;
    await new Promise((r) => setTimeout(r, 150));
  }
  throw new Error(`nedbd-v2 did not come up on :${port}`);
});
after(() => { child?.kill("SIGTERM"); });

const skip = bin ? false : `nedbd-v2 binary not available for ${process.platform}/${process.arch} — integration tests skipped`;
